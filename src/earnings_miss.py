...
# Other defaults
OUTPUT_BASENAME = "./earnings_miss_puts"
...
def main():
    ...
    viable_puts = get_earnings_miss_puts ( symbol_list, args )

    if not viable_puts:
        logging.info( "No earnings miss puts detected" )
        print "No earnings miss puts detected"
        exit ( 0 )
    ...
    for put in viable_puts:
        (symbol,price,pe_ratio,div_yield,div_date,exp_date,strike,ask) = put

        price_at_target = float(price) * target_price_factor
        gain_at_target = strike - price_at_target
